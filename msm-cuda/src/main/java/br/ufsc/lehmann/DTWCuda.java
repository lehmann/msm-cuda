...
public class DTWCuda {
	...
	public static void main(String[] args) throws IOException {
		...
		DTWCuda dtwCuda = new DTWCuda();
		dtwCuda.init();
		System.out.println(dtwCuda.getDistance(t1, t1));
		System.out.println("CUDA's time: " + TimeUnit.NANOSECONDS.toMillis((System.nanoTime() - start)));
		ExecutionStats executionStats = dtwCuda.getDistanceStats(t1, t1, 1.0);
	...
	public void init() throws IOException {
		...
		ptxFileName = preparePtxFile("C:/Users/André/workspace/dtw-cuda/target/classes/DTWCuda.cu");
	...
	public double getDistance(Trajectory A, Trajectory B, double warping) throws IOException {
		double[][] p, q;
		if (A.length() >= B.length()) {
			p = toTrajectoryArray(A);
			q = toTrajectoryArray(B);
